...
const ServiceCard = ({ icon, title, description, gradient, features }) => {
  return (
    <div className="group relative h-full">
      {/* Hover Glow Effect */}
      <div className={`absolute -inset-0.5 bg-linear-to-br ${gradient} rounded-3xl opacity-0 group-hover:opacity-100 blur-xl transition-all duration-500`}></div>
      ...
      <div className="relative h-full bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-3xl p-8 overflow-hidden transition-all duration-500 group-hover:border-white/30 group-hover:transform group-hover:-translate-y-2">
        ...
        <div className="relative space-y-6">
          ...
          <div className="relative inline-flex">
            {/* Animated ring */}
            <div className={`absolute inset-0 bg-linear-to-br ${gradient} rounded-2xl opacity-20 group-hover:opacity-40 transition-opacity duration-500`}></div>
            <div className={`absolute -inset-1 bg-linear-to-br ${gradient} rounded-2xl blur-md opacity-0 group-hover:opacity-60 transition-opacity duration-500`}></div>
            
            {/* Icon box */}
            <div className={`relative w-16 h-16 bg-slate-800/50 backdrop-blur-sm border-2 border-white/20 rounded-xl flex items-center justify-center text-4xl group-hover:border-white/40 group-hover:bg-linear-to-br group-hover:${gradient} transition-all duration-500 shadow-lg`}>
          ...
          {features && features.length > 0 && (
            <ul className="space-y-3 pt-4 border-t border-white/10">
              {features.map((feature, idx) => (
                ...
                >
                  <div className={`w-6 h-6 bg-linear-to-br ${gradient} rounded-lg mr-3 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300`}>
  ...
  >
    ...
    <svg className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" stroke="currentColor">
      <path d="M13 7l5 5m0 0l-5 5m5-5H6"></path>
        ...
        {/* Corner Accent */}
        <div className={`absolute top-0 right-0 w-32 h-32 bg-linear-to-br ${gradient} opacity-0 group-hover:opacity-20 blur-2xl rounded-full transition-opacity duration-500`}></div>
        <div className={`absolute bottom-0 left-0 w-24 h-24 bg-linear-to-tr ${gradient} opacity-0 group-hover:opacity-20 blur-2xl rounded-full transition-opacity duration-500`}></div>
      </div>
    </div>
  )
}
