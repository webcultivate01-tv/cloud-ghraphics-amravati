...
const HeroSection = () => {
  const contactMethods = [
    ...
    { icon: "🕒", label: "Mon-Sat 10-7", color: "purple" },
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="text-center mb-10 sm:mb-12 lg:mb-16">
          ...
          <div className="inline-flex items-center gap-2 bg-violet-500/10 border border-violet-500/20 rounded-full px-3 sm:px-5 py-1.5 sm:py-2 mb-6 sm:mb-8 backdrop-blur-sm">
            ...
            <span className="text-violet-300 text-xs sm:text-sm font-medium">Let's Connect</span>
          </div>
          ...
          <p className="text-gray-400 text-sm sm:text-base md:text-lg lg:text-xl max-w-3xl mx-auto leading-relaxed mb-8 sm:mb-10 lg:mb-12 px-4">
            Ready to bring your creative vision to life? Reach out and let's create something 
            amazing together. We're here to help every step of the way.
          </p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 max-w-5xl mx-auto">
            {contactMethods.map((method, index) => (
              <div
                key={index}
                className={`group relative bg-slate-900/40 backdrop-blur-sm border border-${method.color}-500/20 hover:border-${method.color}-500/50 rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-6 transition-all duration-500 hover:scale-110 hover:-translate-y-2 cursor-pointer`}
                style={{
              ...
              >
                ...
                <div className={`absolute inset-0 bg-linear-to-br from-${method.color}-500/0 to-${method.color}-500/0 group-hover:from-${method.color}-500/10 group-hover:to-${method.color}-500/5 rounded-xl sm:rounded-2xl transition-all duration-500`}></div>
                ...
                <div className="relative">
                  <p className={`text-${method.color}-300 text-xs sm:text-sm font-semibold group-hover:text-${method.color}-200 transition-colors`}>
                    {method.label}
                  </p>
                </div>

                {/* Decorative Corner */}
                <div className={`absolute top-1.5 sm:top-2 right-1.5 sm:right-2 w-1.5 sm:w-2 h-1.5 sm:h-2 bg-${method.color}-500/50 rounded-full group-hover:scale-150 transition-transform duration-500`}></div>
              </div>
            ))}
          </div>
        </div>
